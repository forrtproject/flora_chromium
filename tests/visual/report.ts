...
${rows.map((r) => `<section><details ${r.changed || r.status === "fail" ? "open" : ""}><summary>${escape(r.name)} — ${escape(r.detail ?? r.status)}</summary><div class="shots">${["before", "actual", "diff"].map((kind) => `<div><h2>${kind === "actual" ? "PR" : kind}</h2>${picture(r.name, kind)}</div>`).join("")}</div></details></section>`).join("")}`);
